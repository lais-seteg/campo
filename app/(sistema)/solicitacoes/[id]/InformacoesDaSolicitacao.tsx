...
// ═══════════════════════════════════════════════════════════════════════
//  AS INFORMAÇÕES DA SOLICITAÇÃO — um componente, dois lugares
//
//  Montado pelo POPUP (`DetalheEmPopup`, o olho das tabelas) e pela PÁGINA
//  `/solicitacoes/[id]`. Um componente e não dois porque isto é o retrato
//  do pedido: duas cópias divergem na primeira alteração, e o que a pessoa
//  vê passaria a depender de por onde ela entrou.
//
//  ── SOMENTE LEITURA ──
//
//  As AÇÕES não estão aqui: editar, cancelar, aprovar, acrescentar
//  equipamento e o checklist ficam na linha da tabela e no rodapé da
//  página. É o mesmo desenho do `modalInfoEquipamento` do Controle de
//  Estoque — "Informações do Equipamento" mostra, não mexe.
//
//  E há um motivo técnico junto: as ações abrem os próprios modais, e modal
//  dentro de modal é onde o foco e a tecla Esc se perdem.
//
//  A única exceção é a liberação da folga, que vem com `liberarFolga` e
//  aparece só onde há ação — na página. No popup ela entra como marca de
//  situação, sem botão.
// ═══════════════════════════════════════════════════════════════════════
...
export function InformacoesDaSolicitacao({
  solicitacao: s,
  projetos,
  catalogo,
  hoteis,
  ...
  alteracoes: readonly SolicitacaoAlteracao[];
  /** Previsto × real é de quem responde por ele — ver `podeVerValores`. */
  verValores: boolean;
  /** Mostra o botão de liberar a folga entre campos. Só na página, onde
   *  ações fazem sentido; no popup a folga aparece como marca. */
  liberarFolga?: boolean;
}) {
  const itemPorId = new Map(catalogo.map((i) => [i.id, i]));
  const hotelPorId = new Map(hoteis.map((h) => [h.id, h]));
  ...
  // linha ficar sem reserva é a folga entre campos (supabase/12).
  ...
  return (
    <>
      <Grade>
        ...
        {/* O resumo do dinheiro no alto segue a mesma regra do bloco de
            Previsto × Real lá embaixo: quem não responde por valor não o vê
            aqui também — senão esconder o bloco não esconderia nada. */}
        {verValores ? (
          <>
        ...
        <Info rotulo="Quem aprova">{liderDaSolicitacao(s, projetos)}</Info>
        {s.aprovado_em ? (
        ...
        <Bloco titulo="3 · Equipamento para campo">
          {aguardandoFolga.length ? (
            <p className="modal-hint" style={{ marginTop: 0 }}>
              {aguardandoFolga.length} item(ns) <strong>aguardando liberação</strong>: estão livres nas
              datas deste campo, e o que segura é a folga de um dia que a reserva guarda antes e depois
              de outro campo.{" "}
              {liberarFolga
                ? "Libere abaixo se der tempo de conferir na volta e separar de novo."
                : "Só o administrativo (ou a Direção) pode liberar, no detalhe do pedido."}
            </p>
          ) : null}
          {/* O quadro de CONFERÊNCIA do papel, coluna por coluna:
              ENT. · TESTE · DEV. · TESTE · AVARIA? — mais a coluna de
              RESERVA, que diz se o item está garantido ou ainda esperando a
              liberação da folga. */}
          <TabelaDeLeitura
            ...
            {s.equipamentos.map((e) => {
              ...
              return (
                <tr key={e.id}>
                  ...
                  <td>{e.quantidade}</td>
                  <td>
                    {/* Item fora do catálogo não reserva nada: não há saldo a
                        disputar, então a coluna não se aplica. */}
                    {!e.item_id ? (
                      "—"
                    ) : pendente ? (
                      ...
                      <span className="table-actions" style={{ justifyContent: "center", gap: ".35rem" }}>
                        ...
                        {/* A liberação fica visível DEPOIS de concedida: é o
                            que permite desfazê-la, e o que mostra que aquele
                            item saiu por decisão de alguém. */}
                        {e.folga_dispensada && liberarFolga && !e.entregue ? (
                          ...
                          <span
                            className="status-badge st-neutro"
                            title="A folga entre campos foi liberada para este item"
                          >
                            folga liberada
                          </span>
                        ) : null}
      ...
      {/* SST é UMA MARCA desde a v3: se aplica ou não se aplica. As seis
          conferências saíram da tela — quando o pedido é ANTIGO e tem alguma
          delas marcada, o histórico continua sendo mostrado, para o detalhe
          não apagar o que já foi conferido. */}
        ...
        <Bloco titulo="Previsto × Real">
          <>
            <TabelaDeLeitura colunas={["Linha", "Previsto", "Real", "Desvio"]}>
              ...
              {/* AVARIA — sem previsto, por definição: ninguém orça quebrar o
                  equipamento. O valor é somado pelo banco a partir das
                  avarias registradas, e é por isso que ele aparece aqui e não
                  numa aba separada: é gasto do campo. */}
      ...
      {s.entrega_data || s.devolucao_data ? (
        <Bloco titulo="Conferência">
          <>
            ...
            {assinaturas.length ? (
              // As quatro assinaturas do papel, na ordem em que acontecem.
              // Bloco vazio aparece de propósito: "sem assinatura
              // registrada" é informação, não ausência de informação.
                  ...
                  return (
                    <div className="assin-vista" key={a.id}>
                      ...
                      <em>
                        ...
                        {/* Quem estava logado ao assinar — é esta coluna que
                            faz a assinatura identificar a pessoa. */}
